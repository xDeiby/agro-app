...
export function getRelLabel(entity: EntityRelated, index: EntityRelated): string {
    ...
    return Object.values(metadata.relData).filter((entitie) => entitie.realIndex === index)[0].nameProp;
}
...
export async function getRelOptions(
    ...
    const result = await Promise.all(
        entities.map(async (index) => (await search.getEntities(index, ["str", "index", "id"])).data)
    );
    ...
    const options = wea.reduce((acc: IDropdownOptions, re) => {
        ...
        const option = {
            value: re.id,
            label: re.str.filter((property) => property.index === StringRelated.GENERIC_NAME)[0].value,
        };
